...
import pandas as pd
import pickle
import difflib  # For symptom matching suggestions
...
# Helper functions
def helper(disease):
    """Fetch disease details like description, precautions, medications, diet, and workouts."""
    desc = description.loc[description['Disease'] == disease, 'Description'].values[0]
    pre = precautions.loc[precautions['Disease'] == disease, ['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values[0]
    med = medications.loc[medications['Disease'] == disease, 'Medication'].tolist()
    die = diets.loc[diets['Disease'] == disease, 'Diet'].tolist()
    wrkout = workout.loc[workout['disease'] == disease, 'workout'].tolist()
    return desc, pre.tolist(), med, die, wrkout
...
def suggest_symptom(input_symptom):
    """Suggest closest matching symptom for user input."""
    matches = difflib.get_close_matches(input_symptom, symptoms_dict.keys(), n=1, cutoff=0.7)
...
def predict():
    if request.method == 'POST':
        ...
        user_symptoms = [sym.strip().lower().replace(" ", "_") for sym in symptoms.split(',')]
        corrected_symptoms = []
        ...
        for symptom in user_symptoms:
            if symptom in symptoms_dict:
                ...
                if suggestion:
                    ...
                    message = f"Symptom '{symptom}' not recognized. Please check your input."
                    return render_template('index.html', message=message)
